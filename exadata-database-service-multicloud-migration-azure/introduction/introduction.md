# Oracle Zero Downtime Migration: Physical Online Migration to Exadata Database Service on Oracle AI Database@Azure

## Introduction
Organizations are increasingly adopting multicloud strategies to take advantage of the unique capabilities offered by different cloud providers. As part of this journey, many customers need to migrate their existing Oracle databases running in on-premises environments or on other hyperscalers to Oracle Database@Azure while minimizing downtime and reducing migration complexity.

**Oracle Zero Downtime Migration (ZDM)** is Oracle's recommended solution for performing online and offline migrations of Oracle databases to Oracle Cloud deployments, including Oracle Database@Azure. ZDM automates much of the migration workflow by orchestrating prerequisite validation, database backup and restore, Data Guard synchronization, and switchover operations, helping organizations perform predictable and repeatable migrations with minimal application downtime.
